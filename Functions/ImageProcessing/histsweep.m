function DAs_pad=histsweep(DAs_pad,DAs_bs,nucr)
clustersize=round(2*pi*nucr^2);
bgratio=1.5; %1.5
...
for ci=1:obnum
    ...
    minimask=imopen(minimask,strel('disk',round(nucr/6),0));
    ...
    while mmi<=numsplitobs
        ...
        if numel(pixidx)>clustersize
            ...
            separatemap=imopen(separatemap,strel('disk',round(nucr/3),0));  %ignore cells that are too small *more stringent because condensed cells are very bright
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            remainmap=imopen(remainmap,strel('disk',round(nucr/2),0));   %was nucr/3 before 4/22/2013
