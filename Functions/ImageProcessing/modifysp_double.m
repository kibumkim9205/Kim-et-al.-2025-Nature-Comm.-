function [newsp,trk_rc,corruptlist,leaveout]=modifysp(wellsp,nucr,dims)
...
for f=1:size(wellsp,3)-1
    ...
    coorfortri=double([wellsp{f}(flo,1:2);wellsp{f+1}(sid,1:2)]);           %gather coordinates for all cells from old frame and new cells from next frame
    ...
    coorfortri=double([wellsp{f+1}(flo,1:2);wellsp{f}(eid,1:2)]);
    temptri=delaunay(coorfortri(:,1),coorfortri(:,2));
    neweid=fsz+(1:esz)';
    for cc=1:esz
...
if conlen>0
    ...
    condis1=squareform(pdist(trk_con(:,[1,2])));                    %returns distances between each row (pairwise) of trk_con for the frame and ending cell
    [dis1row,dis1col]=find(condis1==0);dis1lo=(dis1row<dis1col);    %find those where frame & ending cell occurs multiple times (i.e. a single cell is attributed more than once to a new split)
...
for cc=1:size(mermtx,1)
    ...
    if (trk_rc(tempmom,1)<=tempori(1))                              %merged cell not born after merging cell's final frame
        ...
        if ((siz_am-siz_bm)/siz_bm>=siz_vr(2))||((sig_am-sig_bm)/sig_bm>=sig_vr(2))     %large increase in size or signal --> true merge
            ...
            %%% mark any cells that merge as well as subsequent daughters as 'corrupted'
...
%%% find cells that get too close to border
bordercorrupted=zeros(size(trk_rc,1),1);
bordermult = 2;
bordermargin = bordermult*nucr;
for i=(oldlabel(~orphanlo & ~mergecorrupted))'
    for f=trk_rc(i,1):trk_rc(i,3)
        if newsp{f}(i,1)<bordermargin | newsp{f}(i,1)>dims(2)-bordermargin | newsp{f}(i,2)<bordermargin | newsp{f}(i,2)>dims(1)-bordermargin
            bordercorrupted(i)=1;
        end
    end
end

%%% track corrupted cells
%corrupted = mergecorrupted | bordercorrupted;
corrupted= mergecorrupted;
obsoletelabel = oldlabel(orphanlo==1);
for cc=1:size(obsoletelabel,1)
    ...
    trk_rc(obsofind,2)=trk_rc(obsofind,5);
    obsofind=find(trk_rc(:,4)==obsoletelabel(cc));      %same for merge
    trk_rc(obsofind,4)=trk_rc(obsofind,5);
end
corruptlistorg = oldlabel(corrupted==1);
corruptlist = corruptlistorg;
leaveout = oldlabel(orphanlo==1);
%%% if debugmode, keep orphans and track them. Otherwise, remove orphans.
debugmode = 1;  % probably always want to track cells for debugging purposes
if ~debugmode
