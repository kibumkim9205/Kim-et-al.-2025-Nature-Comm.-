function [tracedata,curdatatracked,tracking,nuc_label]=adaptivetrack_8_split(tf,lastgoodframe,curframe,tracedata,curdata,tracking,nuc_raw,nuc_label,nucr,jitter,maxjump,debrisarea,H2BvsNLS,debugpackage)
%masschangethreshold=0.20; %MCF10A-10xBin1=0.20  BJ5-10xBin2=0.10
masschangethreshold=0.20; %default:0.20
areachangethreshold=0.20;
if H2BvsNLS==1
    daughtermin=-0.55;
    daughtermax=-0.45;
elseif H2BvsNLS==2
...
%     if abs(xprev(p)-467)<40 && abs(yprev(p)-1247)<40 && tf==10
%         keyboard;
%     end
    ...
    elseif ~ongoingmerge(p) && areadiff(1)>debrisarea && areadiffnorm(1)>areachangethreshold
