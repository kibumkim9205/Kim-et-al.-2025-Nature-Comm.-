function DAs_pad=getnucmask_histanddeflection(DAs_bs,nucr,histthresh,deflectionthresh)
...
bigthresh=round(histthresh*pi*nucr^2);
onlybigs=bwareaopen(DAs_pad,bigthresh);
remainder=DAs_pad-onlybigs;
%%% perform histogram sweep %%%%%%
DAs_bs=padarray(DAs_bs,[1 1]);
onlybigs=histsweep(onlybigs,DAs_bs,nucr);      %segment nuclei by intensity distribution
onlybigs=imfill(onlybigs,'holes');
onlybigs=imopen(onlybigs,strel('disk',debrisrad,0));
DAs_pad=remainder+onlybigs;
%%% repeat size screen for border detection %%%%%%
bigthresh=round(deflectionthresh*pi*nucr^2);
onlybigs=bwareaopen(DAs_pad,bigthresh);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
