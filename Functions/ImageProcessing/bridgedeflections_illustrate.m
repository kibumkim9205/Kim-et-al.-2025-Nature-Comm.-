function [nuclei,deflections,bridges]=bridgedeflections_illustrate(orgmask,nucr)
[B,L]=bwboundaries(orgmask,'noholes');
obnum=max(L(:));
deflections=zeros(size(orgmask));
bridges=zeros(size(orgmask));
for ci=1:obnum
    orderedset=B{ci};
    orderedset=[orderedset(end:-1:1,2) orderedset(end:-1:1,1)];
    [deflections,bridges]=splitdeflections_5(orderedset,deflections,bridges,nucr);
end
deflections=logical(deflections);
bridges=logical(bridges);
nuclei=orgmask & ~bridges;
nuclei=~bwmorph(~nuclei,'diag');
end

function [alldeflections,bridges,bridgeflag]=splitdeflections_5(orderedset,alldeflections,bridges,nucr)
...
currdeflections=orderedset(vIdx,:);
for adc=1:size(currdeflections,1)
    alldeflections(currdeflections(adc,2),currdeflections(adc,1))=1;
end
