function [tracedata,genealogy,jitters]=postprocessing(tracedata,cellid,jitters,badframes,tracking,maxcellnum,nucr,H2BvsNLS)
...
for i=1:numel(fin)
    ...
    c=find(ostart-finend(i)<4 & ostart-finend(i)>0 & abs(ox-tx)<winrad & abs(oy-ty)<winrad & abs((omass-tmass)/tmass)<percdiff);
    if numel(c)>0
