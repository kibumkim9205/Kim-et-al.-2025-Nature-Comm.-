function [bestsp,best_rc]=rmduplicates(newsp,trk_rc,nucr)
...
for f1=1:scombi
    ...
    TRK1lo=find((best_rc(:,1)==combi(f1,1))&(best_rc(:,3)==combi(f1,2)));       %all cells that run from given start and stop
    ...
    for f2=f1:scombi
        ...
        TRK2lo=find((best_rc(:,1)==combi(f2,1))&(best_rc(:,3)==combi(f2,2)));   %all cells that start and stop in later tracks
