function [tracked,newdaughters,nuc_label]=adaptivetrack_closest_unless(prevdata,curdata,nuc_raw,nuc_label,nucr,extractmask,jitx,jity)
...
distances=zeros(numprev,2);
masschanges=zeros(numprev,2);
for i=1:numprev
    ...
    if sum(daughtercheck)==2
        ...
        prevmatch=[prevmatch;[candidates,[NaN;NaN]]]; %only used to resolve conflicts
        distances=[distances;[0,NaN;0,NaN]];        %daughter call always wins
        masschanges=[masschanges;[0,NaN;0,NaN]];
        continue;
    end
    ...
    elseif numel(consistencycheck)==2
...
tempsort=sortrows(prevmatch);
curconflicts=unique(tempsort(logical([diff(tempsort(:,1))==0;0]),1));
for i=curconflicts
