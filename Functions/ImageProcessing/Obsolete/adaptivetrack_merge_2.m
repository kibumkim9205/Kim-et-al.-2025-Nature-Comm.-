function [tracked,newdaughters,nuc_label,mergenew]=adaptivetrack_merge(lastgoodframe,alldata,curdata,nuc_raw,nuc_label,nucr,jitter,mergeorg)
...
for i=1:numprevorg
    ...
    [~,cidx]=min(sqrt((xcur(neighbors)-mergex).^2+(ycur(neighbors)-mergey).^2));
    match=neighbors(cidx);
    massdiff=(masscur(match)-mergemass)/mergemass;
    if massdiff>masschangethreshold || mergeorg(i)==1
