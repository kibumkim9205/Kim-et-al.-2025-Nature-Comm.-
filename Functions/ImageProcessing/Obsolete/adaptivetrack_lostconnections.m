function [tracked,newdaughters,nuc_label,percentlost]=adaptivetrack(prevdata,curdata,nuc_raw,nuc_label,nucr,jitter,extractmask,reljitx,reljity)
...
[xmax,ymax]=size(extractmask);
inbounds=ones(numprev,1)*NaN;
connected=zeros(numprev,1);
for i=1:numprev
    inbounds(i)=xprev(i)>winrad & xprev(i)<(xmax-winrad) & yprev(i)>winrad & yprev(i)<(ymax-winrad);
...
for i=curconflicts'
    ...
    if numel(conflictdaughter)==1 %single daughter should win
        ...
        connected(prevcells(prevcells~=conflictdaughter))=0;
    elseif numel(conflictdaughter)>1 %must remove the newdaughters assignment
        newdaughters(i)=NaN;
        prevmatch(prevcells)=NaN;
        connected(prevcells)=0;
    else
        prevmatch(prevcells)=NaN;
        connected(prevcells)=0;
    end
end
%%% calculate number of previous cells that failed to be tracked %%%%%%%%%%
inandlost=(~connected).*inbounds;
percentlost=100*sum(inandlost)/sum(inbounds);
%numlost=sum(isnan(prevmatch))-round(sum(~isnan(newdaughters))/2);
%percentlost=100*numlost/numprev;
%%% update tracked info %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
