function [tracked,newdaughters,nuc_label,mergenew]=adaptivetrack_merge(lastgoodframe,curframe,alldata,curdata,nuc_raw,nuc_label,nucr,jitter,mergeorg)
...
for i=1:numprev
    ...
    if massdiff>masschangethreshold || mergeorg(previd(i),1)==1
...
for i=1:numprev
    ...
    if isempty(neighbors)
        removemerge(p)=mergeorg(p,1)>0; %stop tracking merge
        continue;
    end
    ...
    splitcheck=massdiff(1)<-masschangethreshold & massdiff(2)<-masschangethreshold & abs(sum(massdiff)+1)<masschangethreshold;
    daughtercheck=massdiff>-0.55 & massdiff<-0.45 & dist<mitosisrad;
    if splitcheck
        newsplits(candidates)=p;
        prevmatch=[prevmatch;candidates];
        mergeorg(p,4)=curframe;
