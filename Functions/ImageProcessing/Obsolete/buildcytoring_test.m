function [DAs_da,finalcytoring,realnuc_la]=buildcytoring_test(DAs_pad,REs_bs,nucr)
...
srmpoint_rp=regionprops(srmpoint_la,'PixelIdxList');
for i=1:length(srmpoint_rp)
    if isempty(srmpoint_rp(i).PixelIdxList)
        srmpoint_rp(i).PixelIdxList=0;
    end
end
srmpoint_rp=cell2mat(struct2cell(srmpoint_rp)');
...
for i=1:numcells
    ...
    if isempty(cellpix)
        ...
        rsmap(i,:)=find(ismember(srmpoint_rp,cellpix))';
    end
end
rsmap(find(lo),:)=[];
...
intensitymap=log(REs_bs);
%intensitymap=REs_bs;
srm_mean=cell2mat(struct2cell(regionprops(srm_la,intensitymap,'MeanIntensity'))');
