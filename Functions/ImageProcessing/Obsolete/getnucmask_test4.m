...
for ci=1:obnum       %ci = 4
    ...
    %gp=round(0.25/kstep);
    %kacc=getcurvature(ksn,gp);    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %[wsn,wsx]=ksdensity(pixints);
    kstep=(max(pixints)-min(pixints))/100;
    kopen=round(0.1/kstep);
    upslope=diff(ksn)>0;
    upslope=imopen(upslope,strel('square',kopen));
    %{
    firstpeak=downslope(1);
    upslope=find(diff(ksn)>0);
