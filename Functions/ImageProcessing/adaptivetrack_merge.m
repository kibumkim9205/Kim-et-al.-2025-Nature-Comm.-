function [tracedata,curdatatracked,tracking,nuc_label]=adaptivetrack_merge(lastgoodframe,curframe,tracedata,curdata,tracking,nuc_raw,nuc_label,nanmask,nucr,jitter,H2BvsNLS)
...
if numnew
    ...
    new_bg=centroidbackground(nanmask,new_center,nuc_raw,nucr,3,0.25,50);
...
for c=curconflict'
    ...
    curmatch(c)=0;
    curtracking(c,:)=NaN;
%     if numel(tempprevs)==1
%         curmatch(c)=1;
%     elseif numel(tempprevs)>1
%         prevmatch(tempprevs)=NaN;
%         curmatch(c)=0;
%         curtracking(c,:)=NaN;
%     else
