...
[dummy,I_DD]=sort(DD);
bg0=xout(Ibg00(I_DD(2)));              %intensity bin of peak with lowest global curvature

TempSeries2=TempSeries((TempSeries>(bg0-gp*tbin))&(TempSeries<(bg0+gp*tbin)));  %narrow search for peak
...
upperbound = xout(Ibg00(I_DD(2)+1))+5*gp*tbin;
%lowerbound = bg-gp*tbin;
TempSeries3=TempSeries((TempSeries>bg)&(TempSeries<upperbound));
...
%th=s1+Ithr*(s2-s1);
maxposcurveidx=find(ng_th==max(ng_th));
th=xout_th(maxposcurveidx);
