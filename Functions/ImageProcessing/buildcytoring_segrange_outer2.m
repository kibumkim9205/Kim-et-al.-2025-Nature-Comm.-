function [DAs_da,realnuc_la,finalcytoring,seg_range,seg_normrange]=buildcytoring_segrange_outer2(DAs_pad,REs_bs,nucr)
...
DAs_pad=imopen(DAs_pad,strel('disk',round(nucr/3),0));
realnuc_la=bwlabel(DAs_pad);
...
cytoring=crm_la-crz_la;                     %define cytoring with label ID
...
%%% determine range & normalized range %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
seg_range=range(ringsegmean,2);
seg_med=nanmedian(ringsegmean,2);
seg_normrange=seg_range./seg_med;
