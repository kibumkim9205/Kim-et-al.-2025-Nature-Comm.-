function [bestsp,best_rc,corruptlist,leaveoutlist]=rmduplicates(newsp,trk_rc,corrupted,nucr,leaveout)
...
for cc=1:size(obsoletelabel,1)
    obsofind=find(best_rc(:,2)==obsoletelabel(cc));      %any splits attributed to removed cells are removed (i.e. set to cell id)
    best_rc(obsofind,2)=best_rc(obsofind,5);
    obsofind=find(best_rc(:,4)==obsoletelabel(cc));      %same for merge
    best_rc(obsofind,4)=best_rc(obsofind,5);
end
debugmode = 1; % probably want to always keep track of org cell id's for debugging purposes
if ~debugmode
