function [minval,badtraces]=getCdk2exitval(sampletraces,sampledist)
...
for i=1:samplesize
    ...
    sigdist=sampledist(i,firstframe(i):lastframe(i));
    heightgate=signal<1.1;
    if length(signal)>50
        heightgate(50:end)=0;
    end
    if max(heightgate)==0
    ...
    [~,idx]=sort(signaldepth,'descend');
    keyboard;
    
    mintime(i)=find(signaldepth==max(signaldepth),1,'first');
    if sigdist(mintime(i))==0
        badtraces(i)=1;
        continue;
    end
