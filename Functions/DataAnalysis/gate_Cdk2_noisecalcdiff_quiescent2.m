...
%%% remove CDK2inc %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[numcells,tracelength]=size(DHBratio);
rising=ones(numcells,1)*NaN;
for i=1:size(tracestats,1)
    if tracestats(i,3)<minlength
        rising(i)=0;
    else
        rising(i)=DHBratio(i,tracestats(i,1)+minlength-1)>0.5;
    end
end

%badtraces=shorttraces | lowsignal | outliers | noisy;
badtraces=shorttraces | lowsignal | outliers | noisy | rising;
