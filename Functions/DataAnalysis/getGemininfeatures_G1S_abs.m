function [risetime,badtraces]=getGemininfeatures_G1S_abs(sampletraces,samplestats)
...
for i=1:samplesize
    ...
    for j=1:numframes-postbuffer
        presentheight=signal(j)<100; %default 0.05
        minfutureheight=min(signal(j+1:end))>signal(j);
        bufferfutureheight=signal(j+postbuffer)>signal(j)+100; %default 0.1
    ...
    sig_revslope=abs(10*getslope_reverse_avg(signal,1:prebuffer));
    %sig_time=(1:length(signal))/tracelength;
    %filterscore=sig_fwdslope-signal+100;
    filterscore=sig_fwdslope-sig_revslope-signal+100;
    ...
    elseif max(signal(prebuffer:end))<100
        risetime(i)=NaN;
    else
        badtraces(i)=1;
    end
...
for i=1:96
    ...
    if badtraces(i)==1
        ...
        plot(midx,round(ymax/2),'rx','markersize',24);
        continue;
