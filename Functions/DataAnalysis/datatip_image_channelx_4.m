...
global rawdir maskdir tracedata jitters plotfignum immunoframe nucr channel channelnames IFchannelname satlims
winrad=5*nucr;    %window radius: default 5
imagescale=4;     %resize image so it's bigger
framesperhr=5; %default 5
%%% setup %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if frame==immunoframe
    filecode=IFchannelname;
end
raw=double(imread([rawdir,filecode,framestring,'.tif']));
%mask=double(imread([maskdir,'nucedge_',framestring,'.tif']));
%raw=double(imread([maskdir,filecode,framestring,'.tif']));
mask=double(imread([maskdir,'nucedge_',framestring,'.tif']));
%%%%% crop image %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
tempimg=mat2gray(imresize(cropmask,imagescale));
[cropheight,cropwidth]=size(tempimg);
cropmask=zeros(cropheight,cropwidth,3);
cropmask(:,:,1)=tempimg;
%cropraw=mat2gray(imresize(cropraw,imagescale),[double(minpix) double(maxpix)]);
%cropraw=imresize(mat2gray(cropraw),imagescale);
cropmask(:,:,2)=mat2gray(imresize(cropraw,imagescale),satlims);
cropmask(:,:,3)=0;
image(cropmask);
