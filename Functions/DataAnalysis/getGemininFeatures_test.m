...
for i=1:samplesize
    ...
    sig_fwdslope_short=10*getslope_forward_avg(signal,1:10);
    altstore(i,firstframe(i):lastframe(i))=sig_fwdslope_short;
    tempsearch=find(sig_fwdslope_short(prebuffer+1:end)>0.075 & signal(prebuffer+1:end)<0.02,1,'first');
...
for i=1:96
    ...
    hold on;
    %plot(risetime(i),sigstore(risetime(i)),'go','markerfacecolor','g','markersize',6);
