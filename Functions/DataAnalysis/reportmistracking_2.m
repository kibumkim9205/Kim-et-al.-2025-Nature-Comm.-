...
for i=1:numel(fin)
    ...
    operc1=100*(omass(c(1))-fmass)/fmass;
    ojx=jitters(ostart(c(1)),1); ojy=jitters(ostart(c(1)),2);
    fprintf('\t\tX%0.0f Y%0.0f M%0.0f  ',ox(c(1))-ojx,oy(c(1))-ojy,operc1);
    if numel(c)==2
        operc2=100*(omass(c(2))-fmass)/fmass;
        fprintf('X%0.0f Y%0.0f M%0.0f',ox(c(2))-ojx,oy(c(2))-ojy,operc2);
    end
    fprintf('\n');
end
