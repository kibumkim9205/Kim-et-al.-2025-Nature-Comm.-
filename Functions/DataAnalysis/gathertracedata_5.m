function [tracedata,genealogy,jitters,tracestats,motherstats,IFdata,samplecellsID]=gathertracedata_5(datadir,shot,motheroption,daughteroption,IFoption)
%%% Load Data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
load([datadir,'tracedata_',shot,'.mat'],'tracedata','genealogy','jitters');
if IFoption
    load([datadir,'IF_',shot,'.mat'],'IFdata','IFjitter');
    tracedata=padarray(tracedata,[0 1 0],'post');
    tracedata(:,end,1)=IFdata(:,1);
    tracedata(:,end,2)=IFdata(:,2);
    jitters=[jitters;IFjitter];
else
...
elseif daughteroption==1
    gatedaughter=daughters; %only traces that start with mitosis
elseif daughteroption==2
    gatedaughter=~daughters; %only traces that don't start with mitosis
end
