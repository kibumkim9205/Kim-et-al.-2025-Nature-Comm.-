...
%   labeled region in the label matrix L. L can be numeric or categorical.
%   When L is numeric, positive integer elements of L correspond to
...
%
%   Perimeter and Circularity should be used on a label matrix or binary 
%   image with contiguous regions. Otherwise, 'Perimeter' gives unexpected 
%   results on discontiguous regions which also affects the result of 
%   'Circularity'. Circularity is also not recommended for very small 
%   regions such as a 3x3 square, as in such cases the results might exceed
%   the circularity value for a perfect circle which is 1.
%
%   If 'MaxFeretProperties' is selected, the function will output the
%   following properties related to the maximum Feret Diameter -
%      MaxFeretDiameter   - Maximum Feret diameter length.
%      MaxFeretAngle      - Angle of maximum Feret diameter with respect to X
%                           axis in degrees.
%      MaxFeretCoordinates- Endpoint coordinates of maximum Feret diameter.
%   
%   If 'MinFeretProperties' is selected, the function will output the
%   following properties related to the minimum Feret Diameter -
%      MinFeretDiameter   - Minimum Feret diameter length.
%      MinFeretAngle      - Angle of minimum Feret diameter with respect to X
%                           axis in degrees.
...
%   returned by BWCONNCOMP. If the first input is L, L must be categorical
%   or numeric array which must be real, nonsparse, and contain integers. L
%   can have any numeric class and any dimension.
%
%   Note on Terminology
%   -------------------
...
for k = 1 : numRequestedStats
    switch requestedStats{k}
        ...
        case 'Circularity'
            [stats, statsAlreadyComputed] = ...
                    ComputeCircularity(L, imageSize, stats, statsAlreadyComputed);
        ...
        case {'MaxFeretProperties', 'MinFeretProperties'}
            [stats, statsAlreadyComputed] = ...
                ComputeFeretProperties(CC, L, imageSize, stats, statsAlreadyComputed);
           
    end
end

% Create the output stats structure array.
outstats = createOutputStatsStructure(requestedStats, stats, isInputCategorical);

% If OUTPUT is 'table' then convert the struct into a table.
if isTableRequested
    
    if isfield(outstats, 'PixelIdxList')
        outstats = struct2table(outstats,'AsArray',true);
        if (~ iscell(outstats.PixelIdxList))
            tmp = table2cell(outstats);
            outstats = table (tmp);
            outstats.Properties.VariableNames = {'PixelIdxList'};
...
%%%
%%% ComputeFeretProperties
%%%
function [stats, statsAlreadyComputed] = ...
    ComputeFeretProperties(CC, L, imageSize, stats, statsAlreadyComputed)

if numel(imageSize) > 2
        NoNDSupport('FeretProperties');
        return
end

if ~statsAlreadyComputed.MaxFeretProperties 
    statsAlreadyComputed.MaxFeretProperties = 1;
    % Calculate Max Feret Properties Using bwferet
    if isempty(L)
        out = bwferet(CC, 'MaxFeretProperties');
    else 
        out = bwferet(L, 'MaxFeretProperties');
    end
    out = table2struct(out);
    for k = 1:length(stats)
        stats(k).MaxFeretDiameter = out(k).MaxDiameter;
        stats(k).MaxFeretAngle = out(k).MaxAngle;
        stats(k).MaxFeretCoordinates = out(k).MaxCoordinates;
    end
end    
if ~statsAlreadyComputed.MinFeretProperties
    statsAlreadyComputed.MinFeretProperties = 1;
    % Calculate Min Feret Properties Using bwferet
    if isempty(L)
        out = bwferet(CC, 'MinFeretProperties');
    else 
        out = bwferet(L, 'MinFeretProperties');
    end
    out = table2struct(out);
    for k = 1:length(stats)
        stats(k).MinFeretDiameter = out(k).MinDiameter;
        stats(k).MinFeretAngle = out(k).MinAngle;
        stats(k).MinFeretCoordinates = out(k).MinCoordinates;
    end
end
...
if ~statsAlreadyComputed.PerimeterOld
    ...
    B = images.internal.builtins.regionboundaries(double(L),8);
...
if ~statsAlreadyComputed.PerimeterOld
    ...
    for k = 1:length(stats)
        B = images.internal.builtins.regionboundaries(double(stats(k).Image),8);
...
function [stats, statsAlreadyComputed] = ...
    ComputeCircularity(L, imageSize, stats, statsAlreadyComputed)
% Circularity not supported for non 2D objects
if ~statsAlreadyComputed.Circularity
    statsAlreadyComputed.Circularity = 1;
    if numel(imageSize) > 2
        NoNDSupport('Circularity');
        return
    end
    % If Area is not already calculated, calculate area
    if ~statsAlreadyComputed.Area
        [stats, statsAlreadyComputed] = ...
        ComputeArea(stats,statsAlreadyComputed);
    end
    % If perimeter is not already calculated, calculate the perimeter
    if ~statsAlreadyComputed.Perimeter
        if ~isempty(L)
           [stats, statsAlreadyComputed] = ...
                ComputePerimeterWithLabelMatrix(L,stats, ...
        ...
        for k = 1:length(stats)
            stats(k).Circularity = 4*pi*stats(k).Area/((stats(k).Perimeter)^2);
        end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function outstats = createOutputStatsStructure(requestedStats, stats, isInputCategorical)
...
% If Feret Properties are requested, remove 'MaxFeretProperties' and
% 'MinFeretProperties' and add individual Feret Properties to the output
% to keep.
FeretProps = {};
if any(strcmp(requestedStats,'MaxFeretProperties'))
        FeretPropsMax = {'MaxFeretDiameter','MaxFeretAngle','MaxFeretCoordinates'};
        FeretProps = [FeretProps,FeretPropsMax];
end
if any(strcmp(requestedStats,'MinFeretProperties'))
        FeretPropsMin = {'MinFeretDiameter','MinFeretAngle','MinFeretCoordinates'};
        FeretProps = [FeretProps,FeretPropsMin];
end
if ~isempty(FeretProps)
    requestedStats = [requestedStats, FeretProps];
    idxRemove = ~ismember(fnames, requestedStats)...
        |strcmp(fnames, 'MaxFeretProperties')...
        |strcmp(fnames, 'MinFeretProperties');
else
    idxRemove = ~ismember(fnames, requestedStats);
end
