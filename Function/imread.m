...
%   specified by the character vector or string scalar FILENAME. FILENAME must be in the current 
%   directory, in a directory on the MATLAB path, or include a full or 
%   relative path to a file.
%   
%   FMT specifies the format of the file by its standard
...
%   from its content.
% 
%   [...] = IMREAD(URL,...) reads an image from an Internet URL or stored
%   at a remote location. When reading data from remote locations, you must
%   specify the full path using a uniform resource locator (URL). For
%   example, to read an image from Amazon S3 cloud specify the full URL for
%   the image:
...
%   three-element vector whose values are in the range [0,1]. The name
...
% 
%   See TIFF, same Special Syntaxes and Notes apply.
%
...
%       imdata = imread('http:://hostname/path_to_file/my_image.jpg');
% 
%       % Read image from an Amazon S3 bucket location
%       imdata = imread('s3://bucketname/path_to_file/my_image.jpg');
%
%   See also IMFINFO, IMWRITE, IMFORMATS, FREAD, IMAGE, DOUBLE, UINT8.

%   Copyright 1984-2020 The MathWorks, Inc.

% Get format cached from the last call to imread. This is a performance
% optimization to avoid repeated calls to imformats while reading multiple
% files with the same extension.
cached_fmt = cachedFormat();
...
try
    ...
    throwAsCaller(MException(errorID, '%s', ME.message));  
end
...
% Set cached format to fmt_s, which is now the latest successfully used
% format
cachedFormat(fmt_s);
...
%--------------------------------------------------------------------------
function [source, fmt_s, extraArgs, was_cached_fmt_used] = parse_inputs(cached_fmt, varargin)
...
[fid, errmsg] = matlab.internal.fopen(filename, 'r');
if (fid == -1)
