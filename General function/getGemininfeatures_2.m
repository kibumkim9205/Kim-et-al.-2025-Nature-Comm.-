function [risetime,badtraces]=getGemininfeatures_2(sampletraces,samplestats,risethresh,quiescentanalysis)
...
for i=1:samplesize
    ...
    for j=1:numframes-futurebuffer
        ...
        gate(j)=presentheight & bufferfutureheight;%minfutureheight &
    end
    gate(1:earlycutoff)=0;
    findFirst=find(gate==1);
    numGate=length(findFirst);
    if numGate>5
        clear secondPeak;
        for idx=2:numGate
            secondPeak(idx)=findFirst(idx)-findFirst(idx-1);
        end
        secondGate=find(secondPeak>10);
        if ~isempty(secondGate)
            gate(findFirst(secondGate):findFirst(end))=0;
        end
    end
