...
% please report any bugs or suggestions to oldnhuis@gmail.com
    ...
    if (narg == 1)
        ...
        parameters = varargin(1+have_settings : 3 : end);
        values     = varargin(2+have_settings : 3 : end);
        status     = varargin(3+have_settings : 3 : end);
    end
        ...
        while true
            ...
            if strcmpi(tags{ii}, 'Separator')
                ...
                if islogical(values{ii})
                    ...
                    % set status
                    if strcmpi(status{ii}, 'off')
                        set(controls(ii), 'enable', 'inactive');
                    end
