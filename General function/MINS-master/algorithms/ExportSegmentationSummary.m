...
try
    imgInfo = bioiminfo(rawfile);
catch exp
    imgInfo.image_num_c = 1;
end

% output headers
strHeader = sprintf('Inlier/Outlier, Embryo Id, Cell ID, Size, TE/ICM, X, Y, Z, ');
for channel = 1:imgInfo.image_num_c
...
nStatsSeg = 8;
nStatsPerChannel = 2;
M = zeros(length(Iobj), nStatsSeg + nStatsPerChannel*imgInfo.image_num_c);
...
for channel = 1:imgInfo.image_num_c
    % load channels
    data = bioimread(rawfile, frame, channel);
