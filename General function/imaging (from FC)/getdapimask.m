...
%% threshold mask
da_bs_g=mat2gray(da_bs);
th_gray=graythresh(da_bs_g);
th_high=min(da_bs(da_bs_g>th_gray));
%[~,th_high]=ThreshImage(da_bs);
[~,th_low]=ThreshImage(da_bs);

da_mas=(da_bs>1/mean([1/th_high,1/th_low]));
...
%% filtering
da_ma=bwareaopen(da_ma1,round(nucr.^2/2));
