function [ImageMask,th,bg]=ThreshImage_smbg(ImageOld)
%for very small background
...
neglo=((DD<=negth)|(AC>=gp));Ibg00=Ibg00(neglo);
if isempty(Ibg00)
    [~,Ibg00]=min(ng);
end
...
dn_bg=diff(n_bg);
Ibg=1+find((dn_bg(1:end-1)>=0) & (dn_bg(2:end)<=0) & (abs(dn_bg(1:end-1))+abs(dn_bg(2:end))>0),1,'first');%better value
...
%% get threshold
TempSeries3=TempSeries((TempSeries>bg-5*gp*tbin)&(TempSeries<xout(Ibg00(2))+10*gp*tbin));
[n_th,xout_th]=ksdensity(TempSeries3,bg:(xout(Ibg00(2))+10*gp*tbin-bg)/100:(xout(Ibg00(2))+10*gp*tbin));
...
% end
[~,I]=max(partng(pospeak(1).PixelIdxList));
Ith00=pospeak(1).PixelIdxList(I);
%[~,I]=max(AC);
%Ith0=floor(Ith00(I));Ithr=(Ith00(I)-Ith0);
Ith0=floor(Ith00);Ithr=(Ith00-Ith0);
