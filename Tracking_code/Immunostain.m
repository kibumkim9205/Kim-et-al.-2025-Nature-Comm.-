...
if row==1:2
    nucr=12; %
    eccentricitythresh=0.9; %0.7-0.97
    debrisarea=200;
    boulderarea=1800;
    segmethod='log';
elseif row==3:4
...
switch FileOption
    case 'nd2'
        ...
        for idxChannel=1:numChannel
            ...
            %showImagesWithLinkedAxes({rawImage{1},rawImage{2},rawImage{3},rawImage{4}})
        end
    case 'tif'
        shot=saveshot;
...
blurA4=imfilter(rawA{4},fspecial('disk',3),'symmetric');
...
[realA4,bgA4]=bgsubmasked_global_2(blurA4,nanmaskcyto,numblocks_1,compression,sampleprctile);
...
if puncta
    ...
    %punctathresh_low=2000; punctathresh_high=4000;
    %[puncta_info_low_A3,punctacount_low_A3]=getpuncta(nuc_label_all,realA3,punctathresh_low);
    %[puncta_info_high_A3,punctacount_high_A3]=getpuncta(nuc_label_all,realA3,punctathresh_high);
end
%%% extract features %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
nanvec=ones(numgood,1)*NaN;
xcoor=nanvec; ycoor=nanvec; nuc_area=nanvec;
sigA1=nanvec; sigA2=nanvec; sigA3=nanvec; sigA4=nanvec;
...
for cc=1:numgood
    ...
    sigA4(cc)=median(realA4(nuc_info(cc).PixelIdxList));    
end
...
%%% store data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
IFdata=[xcoor,ycoor,nuc_area,sigA1,sigA2,sigA3,sigA4];
